<template>
  <div>
    ...
    <div class="entity-card-container">
      <el-card class="entity-card" v-for="entityType in speakerEntityTypes" :key="entityType.id">
        <div>
          <div>模型名称：{{entityType.entity_type_name}}</div>
          <div>会议数量：{{entityType.entity_instance_count}}</div>
        </div>
        <router-link :to="{name: 'entitySpeakerDetail', params: {entityTypeId: entityType.id}}">
          查看
        </router-link>
      </el-card>
    </div>
    ...
    methods: {
      ...
      loadSpeakerEntity () {
        this.axios.get('/api/entities/speaker').then(response => {
          this.speakerEntityTypes = response['data']
        }, response => {
          this.$message(response['response']['data']['message'])
        })
      }
